package br.com.mauriciomachado.todolist.task;
...
import br.com.mauriciomachado.todolist.utils.Utils;
...
public class TaskController {
    ...
    public ResponseEntity create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if(currentDate.isAfter(taskModel.getStartAt()) || currentDate.isAfter(taskModel.getEndAt())){
         ...
         if(taskModel.getStartAt().isAfter(taskModel.getEndAt()) ){
             return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("A data de iníciodeve ser menor do que a data de término ");
        }
    ...
    public List<TaskModel> list(HttpServletRequest request) {
     ...
     // http://localhost:8080/tasks/897s87sd-asdsdsd-8s87d8s7d
     @PutMapping("/{id}")
